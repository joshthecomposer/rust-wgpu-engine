// WebGL2 composite: GLES could `texture()` a depth Texture2D raw; WGSL samples of
// `texture_depth_2d` do not lower to legal GLSL ES `texture()` on all drivers.
//
// Fragments export `frag_pos.z` into MRT `.r` (see `*_wasm.wgsl` shaders).
// — same numeric space as GLES `sampler2D uDepth`; fog matches `hdr_composite.wgsl`.
...
// Normal color texture: .r holds depth buffer–equivalent fragment Z ([0–1], same as GLES).
@group(0) @binding(3) var depth_lin: texture_2d<f32>;
