...
const float fogStart = 10.0;   // in view-space units
const float fogEnd = 85.0;     // in view-space units
const float fogStrength = 0.95; // 0..1 overall multiplier

float viewZFromDepth(float depth01, vec2 uv)
{
    float z = depth01 * 2.0 - 1.0; // back to NDC z [-1..1]
