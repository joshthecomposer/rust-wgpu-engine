...
layout (location = 6) in mat4 i_model; // locations 6..9
...
// binding can be whatever you like; just match in Rust
...
void main() {
    ...
    for (int i = 0; i < MAX_BONE_INFLUENCE; i++) {
        ...
        if (bid >= MAX_BONES) { totalPosition = vec4(a_pos,1.0); break; }

        mat4 B = bones[base + bid];
        totalPosition += (B * vec4(a_pos, 1.0)) * w;

        mat3 BN = transpose(inverse(mat3(B)));
        totalNormal += (BN * a_normal) * w;
    }

    vec4 world = i_model * totalPosition;
    FragPos = world.xyz;
    FragPosLightSpace = light_space_mat * vec4(FragPos, 1.0);

    mat3 nrm = mat3(transpose(inverse(i_model)));
    Normal = normalize(nrm * totalNormal);
...
vec4 calculate_directional_light() {
	...
	vec3 ambient = mix(flat_ambient, envAmbient, 0.42);
