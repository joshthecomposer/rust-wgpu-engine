...
fn handle_player_movement_rapier(
    ...
    let Some(check_key) = player_keys.first() else {
        return;
    };
    ...
    let rb = ps
        .rigid_body_set
        .get_mut(physics_handle.rigid_body)
        .unwrap();
    ...
    if input_state.keys_current.contains(&KeyCode::KeyW) {
        move_dir += forward_flat;
    }
    if input_state.keys_current.contains(&KeyCode::KeyS) {
        move_dir -= forward_flat;
    }
    if input_state.keys_current.contains(&KeyCode::KeyD) {
        move_dir += right_flat;
    }
    if input_state.keys_current.contains(&KeyCode::KeyA) {
        move_dir -= right_flat;
    }
...
fn handle_enemy_movement_rapier(
    ...
    for id in ids {
        ...
        let Some(dest) = em.destinations.get(id) else {
            continue;
        };
        ...
        if animator.next_animation == AnimationType::Death
            ...
            || sim_controller.state == SimState::Dead
        {
            continue;
        };

        if animator.next_animation == AnimationType::Flinch {
            continue;
        };

        if sim_controller.state == SimState::Combat {
            continue;
        };

        em.v_effects.remove(id);

        if *ent_type == "MooseMan" {
            continue;
        };
        ...
        if let Some(transform) = em.transforms.get_mut(id) {
            ...
            transform.rotation =
                Quat::from_array(iso.rotation.coords.as_slice().try_into().unwrap());
        }
    }
}
