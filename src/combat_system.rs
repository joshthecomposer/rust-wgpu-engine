...
fn handle_player_to_enemy(
    ...
    if let Some(anim) = animator.get_next_animation() {
        if let Some(activation_data) = &anim.hurtbox_activation {
            if activation_data.triggered.get() {
                hitset.clear();
            }
        }
    }
...
//                            did_particles: false,
//                        };
//                        if player_state.state != PlayerState::Defensive {
//                            if let Some(h) = em.healths.get_mut(target_id) {
//                                *h -= 50.0
//                            };
//
