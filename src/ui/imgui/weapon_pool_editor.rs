        ...
        ui.window("Weapon Pool Editor")
            ...
            .build(|| {
                ...
                let mut entity_types: Vec<String> =
                    em.entity_type_register.keys().cloned().collect();
                entity_types.sort_unstable();

                ui.combo(
                    "Entity Type",
                    &mut self.selected_entity_type_idx,
                    &entity_types,
                    |s| Cow::Borrowed(s),
                );

                let mut categories: Vec<String> = em
                    .weapon_pools_config
                    .weapon_types
                    .keys()
                    .cloned()
                ...
                if let Some(entity_type) = entity_types.get(self.selected_entity_type_idx) {
                    ui.combo(
                        "Weapon Category",
                        &mut self.selected_category_idx,
                        &categories,
                        |s| Cow::Borrowed(s),
                    );
                    ...
                    if ui.button("Apply Mapping") {
                        if let Some(cat) = categories.get(self.selected_category_idx) {
                            em.weapon_pools_config
                                .save_to_file("config/weapon_pools_config.json");
                        }
                    }
                }
                ...
                ui.input_text("New Category Name", &mut self.new_category_name)
                    .build();
                if ui.button("Create Category") {
                    if !self.new_category_name.is_empty()
                        && !em
                            .weapon_pools_config
                            .weapon_types
                            .contains_key(&self.new_category_name)
                    {
                ...
                ui.combo(
                    "Edit Category",
                    &mut self.edit_category_idx,
                    &categories,
                    |s| Cow::Borrowed(s),
                );

                if let Some(category_name) = categories.get(self.edit_category_idx) {
                    if let Some(weapon_type) =
                        em.weapon_pools_config.weapon_types.get_mut(category_name)
                    {
                        ui.text(format!("Editing: {}", category_name));

                        let mut abilities: Vec<(AbilityId, String)> = em
                            .abilities_config
                            .abilities
                            .iter()
